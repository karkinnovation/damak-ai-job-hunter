...
async function createJob(formData: FormData) {
  ...
  const { data: business } = await supabase.from('businesses')
    .select('id,ward,city,district,province,latitude,longitude')
  ...
  const parsed = jobSchema.safeParse({
    ...
    experience_required_months: formData.get('experience_required_months'),
    education_requirement: formData.get('education_requirement') || '',
    working_start: formData.get('working_start'),
    working_end: formData.get('working_end'),
    number_of_openings: formData.get('number_of_openings'),
    required_skills: formData.getAll('required_skills').map(String),
    preferred_skills: formData.getAll('preferred_skills').map(String),
  })
  ...
  const { error } = await supabase.from('jobs').insert({
    ...
    experience_required_months:d.experience_required_months,
    education_requirement:d.education_requirement || null,
    working_start:d.working_start,
    working_end:d.working_end,
    number_of_openings:d.number_of_openings,
...
export default async function NewJob({ searchParams }: { searchParams: Promise<{error?:string}> }) {
  ...
  return <section className="narrow">
    <span className="eyebrow">Employer · New vacancy</span>
    <h1>Post a job anywhere in Nepal</h1>
    <p className="muted">The vacancy automatically uses your Business Profile location, so employers do not have to enter the same address again.</p>
    {!business && <p className="notice">Create your business profile first. <a href="/employer/profile"><b>Open profile →</b></a></p>}
    {error && <p className="error">{error}</p>}
    <form className="stack card" action={createJob}>
      {business && <div className="notice">📍 Workplace: <strong>{locationLabel(business)}</strong> · Change this from your Business Profile.</div>}
      ...
      <div className="field"><label>Category</label><select name="category">{JOB_CATEGORIES.map(c => <option key={c}>{c}</option>)}</select></div>
