...
export default async function Dashboard() {
  ...
  if (!profile) return <section className="narrow"><div className="card"><h1>Account setup problem</h1><p>Your profile row is missing. Run the supplied Supabase migration and sign in again.</p></div></section>

  if (profile.role === 'job_seeker') {
    const [{ data: seeker }, { count: applications }] = await Promise.all([
  ...
  if (profile.role === 'employer') {
    ...
    return <section className="container"><div className="sectionHeader"><div><span className="eyebrow">Employer dashboard</span><h1>{business?.business_name || profile.full_name}</h1><p className="muted">Post local vacancies and review applicants ranked by compatibility.</p></div><Link className="button" href="/employer/jobs/new">Post vacancy</Link></div>
      <div className="grid"><div className="card kpi"><span className="muted">Business profile</span><span className="stat">{business ? 'Ready' : 'Incomplete'}</span><Link href="/employer/profile">Edit profile →</Link></div><div className="card kpi"><span className="muted">Vacancies</span><span className="stat">{jobs || 0}</span><Link href="/employer/jobs">Manage vacancies →</Link></div><div className="card kpi"><span className="muted">Hiring</span><span className="stat">AI-ranked</span><span className="muted">Recommendations never auto-reject applicants.</span></div></div>
    </section>
  }
